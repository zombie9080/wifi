...
void http_send_redirect_to_advertisement(request * r, const char *urlFragment, const char *text)
{
    ...
    if(NULL == urlFragment)
    {
        ...
        //sprintf(url,"http://localhost:8888/?url=http://192.168.1.246:2060");
...
#if 1
void wifidog_autoauth_to_authpuppy(request * r, char *urlfra ,char * original_url)
{
    char *url = (char *)malloc(1024);
    int sockfd; 
    char buf[MAX_BUF];
    ...
    inet_pton(AF_INET,config->gw_address,&wifi_addr.sin_addr);
    ...
    sprintf(url,"GET /%s HTTP/1.1\r\n"
                ...
                "\r\n",
                urlfra,auth_server->last_ip
                );
    sockfd = connect_auth_server();
    res = http_get(sockfd, url);
    sub = strstr(res,"authpuppy=");
    if(sub)
    {
        snprintf(cookie,27,sub+10);
        printf("COOKIE-1:%s\n",cookie);
    }
    ...
    sprintf(url,"POST /%s HTTP/1.1\r\n"
                ...
                "Connection: keep-alive\r\n"
                "\r\n",
                urlfra,auth_server->last_ip,cookie
           );
    sockfd = connect_auth_server();
    res = http_get(sockfd, url);
    bzero(cookie,32);
    sub = strstr(res,"authpuppy=");
    snprintf(cookie,27,sub+10);
    sub = strstr(sub+37,"authpuppy=");
    if(sub)
    {
        snprintf(cookie,27,sub+10);
        debug(LOG_INFO,"double cookie");
    }
    printf("COOKIE-2:%s\n",cookie);
...
#if 3
    bzero(url,1024);
    sprintf(url,"GET /wifidog/auth?token=%s HTTP/1.1\r\n"
                "Host: %s:%d\r\n"
                "Connection: keep-alive\r\n"
                "\r\n",
                token,config->gw_address,config->gw_port
            );
    printf("URL3:%s\n",url);
    res = http_get(wifi_sock, url);
    ...
    sprintf(url,
            ...
            "Connection: keep-alive\r\n"
            "\r\n",
            *gw_id?gw_id:config->gw_id,auth_server->last_ip,cookie
            ,auth_server->last_ip,cookie
            );
    sockfd = connect_auth_server();
    res = http_get(sockfd, url);
    debug(LOG_INFO, "RESPONSE4: %s", res);
    free(url);
    sub = strstr(res,"Location: ");
    if( NULL == sub )
    {
        printf("sub is null\n");
        strcpy(final_url,original_url);
    }
    else
    {
...
#endif
#if 0
    http_send_redirect_to_advertisement(r,final_url,NULL);
#else
...
#else
        //pthread_t tid;
        //pthread_create(&tid,NULL,wifidog_autoauth_to_authpuppy,);
        wifidog_autoauth_to_authpuppy(r,urlFragment,tmp_url);
#endif
        free(urlFragment);
    }
    free(url);
...
http_callback_auth(httpd * webserver, request * r)
{
    ...
    char *mac;
    // add by zp
    debug(LOG_INFO,"goto http_callback_auth()");
    ...
    if ((token = httpdGetVariableByName(r, "token"))) {
        ...
        //if (!(mac = arp_get(r->clientAddr))) {
            ...
            if (!logout) { /* applies for case 1 and 3 from above if */
                // add by zp
                debug(LOG_INFO,"goto authenticate_client()");
